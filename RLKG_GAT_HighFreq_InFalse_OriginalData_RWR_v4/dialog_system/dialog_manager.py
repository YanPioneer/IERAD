...
class DialogManager:
    ...
    def initialize_episode(self):
        """ start a new episode """
        # print("New Episode!")
        self.disease_mask = copy.deepcopy(dialog_config.disease_mask).to(dialog_config.device).reshape(-1, 12)
        self.symptoms_mask = copy.deepcopy(dialog_config.symptoms_mask).to(dialog_config.device).reshape(-1, 118)

        # print(self.disease_mask)
        dialog_config.action_re = True
        disease_rest = torch.where(self.disease_mask > 0.,
                                   torch.ones(self.disease_mask.size()).to(dialog_config.device),
                                   torch.zeros(self.disease_mask.size()).to(dialog_config.device))
        # print(disease_rest)
        disease_rest_ = torch.sum(disease_rest)
        # print(disease_rest_)
        sym_rest = torch.where(self.symptoms_mask > 0.,
                               torch.ones(self.symptoms_mask.size()).to(dialog_config.device),
                               torch.zeros(self.symptoms_mask.size()).to(dialog_config.device))
        # print(sym_rest)
        ...
        with open(dialog_config.action_rest_re, 'a+', encoding='utf-8') as ref:
            ref.write('\nNew Episode!\n')
            ref.write('start action:' + str(action_rest) + '\n')
