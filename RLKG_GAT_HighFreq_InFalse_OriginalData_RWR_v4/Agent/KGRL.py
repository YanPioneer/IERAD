...
class KGRL(nn.Module):
    ...
    def __init__(self, GraphAgent, DisInfer, input_shape, sym_shape, dis_shape, hidden_size, slot_set, entity_num, embed_size, threshold, device, max_turn):
    ...
    def construct_graph(self, disease_mask):
        dis_sym_num = dialog_config.dis_sym_num_to_graph
        dis = dialog_config.disease
        sym = dialog_config.symptoms
        rwr_all = torch.zeros(disease_mask.size(0), len(dis), len(sym)).to(self.device)
        for b in range(disease_mask.size(0)):
            ...
            for i in range(len(dis_mask)):
                if dis_mask[i] > 0.:
                    dis_exit.append(dis[i])
            for d in dis_sym_num.keys():
                if d in dis_exit:
                    for s in dis_sym_num[d].keys():
                        weight_edges.append((d, s, dis_sym_num[d][s]))
            nodes = dis + sym
            rwr_ = {}
            for d in dis:
                rwr_[d] = {}
                ...
                # print(G.edges)
                a = nx.pagerank(G, personalization={d: 1})
                rwr_[d] = a
            rwr_matrix = torch.zeros((len(dis), len(sym)))
            rwr_matrix_ = torch.zeros((len(dis), len(sym)))
            for d in dis:
                for s in sym:
                    rwr_matrix[dis.index(d)][sym.index(s)] = rwr_[d][s]
            # print(len(rwr_matrix))
            for i in range(len(dis_exit)):
                for j in range(len(sym)):
                    rwr_matrix_[dis.index(dis_exit[i])][j] = rwr_matrix[dis.index(dis_exit[i])][j] / sum(rwr_matrix[dis.index(dis_exit[i])])
            # print(rwr_matrix_)
            # print(sum(rwr_matrix_[0]))
            rwr_all[b] = copy.deepcopy(torch.tensor(rwr_matrix_))
        return rwr_all

    def filter_sig(self, rwr_p):
        for b in range(rwr_p.size(0)):
            for i in range(len(rwr_p[b])):
                if rwr_p[b][i] > 0.:
                    rwr_p[b][i] = F.sigmoid(rwr_p[b][i])
        return rwr_p
    ...
    def forward(self, dialog_state, dialog_sym_rep, kg_adj, sym_flag, sym_mask, disease_mask, symptoms_mask):
        ...
        sym_rwr = self.construct_graph(disease_mask)
        d_ = copy.deepcopy(disease_dis).unsqueeze(1)
        sym_rwr_prob = torch.matmul(d_, sym_rwr).squeeze(1)  # rwr
        sym_rwr_prob = self.filter_sig(sym_rwr_prob)
        symptom_p = F.sigmoid(self.Sym_Inquiry(state))  # RL
        sym_prob = mu * symptom_p + (torch.ones_like(mu) - mu) * sym_rwr_prob
        symptom_ = sym_prob * sym_flag
        symptom_final = symptom_ * symptoms_mask

        if dialog_config.action_re:
            # 记录RL与RWR选择的动作与mu
            with open(dialog_config.test_re, 'a+', encoding='utf-8') as ref:
                ref.write('*********\n')
                ref.write('RL_action:' + str(symptom_p) + '\n')
                ref.write('RWR_action:' + str(sym_rwr_prob) + '\n')
                ref.write('mu:' + str(mu) + '\n')
                ref.write('symptom_final:' + str(symptom_final) + '\n')
                ref.write('*********\n')
            # 记录剩余动作，用于实验分析
            # print(disease_dis)
            disease_rest = torch.where(disease_dis > 0.,
                                       torch.ones(disease_dis.size()).to(dialog_config.device),
                                       torch.zeros(disease_dis.size()).to(dialog_config.device))
            # print(disease_rest)
            disease_rest_ = torch.sum(disease_rest)
            # print(disease_rest_)
            # print(symptom_final)
            sym_rest = torch.where(symptom_final > 0.,
                                   torch.ones(symptom_final.size()).to(dialog_config.device),
                                   torch.zeros(symptom_final.size()).to(dialog_config.device))
            # print(sym_rest)
            sym_rest_ = torch.sum(sym_rest)
            # print(sym_rest_)
            action_rest = disease_rest_ + sym_rest_
            with open(dialog_config.action_rest_re, 'a+', encoding='utf-8') as ref:
                ref.write('step action:' + str(action_rest) + '\n')
    ...
    def predict(self, kg_matrix, state, dialog_sym_rep, sym_flag, sym_mask, turn, disease_mask, symptoms_mask):
        ...
        if (dis_pro > self.threshold and turn > 2) or avaliable_dis == 1 or turn == self.max_turn or dialog_config.include_sym_is_empty:
        # if (dis_pro > self.threshold and turn > 5) or avaliable_dis == 1 or turn == self.max_turn:  # 训练 疾病置信度高；症状的概率都不足以询问；到达最大会话次数；均直接诊断
...
class Agent(nn.Module):
    ...
    def __init__(self, GraphAgent, DisInfer, input_shape, sym_shape, dis_shape, hidden_size, slot_set, slot_list, entity_num, embed_size,
                 threshold, epsilon, replaybuffer, req_dise_sym_dict, dise_sym_num_dict, dis_path, gat_path, lr, batch_size, target_net_update_freq, warm_start, buffer_size, discount, max_turn):
        ...
        self.model = KGRL(GraphAgent, DisInfer, input_shape, sym_shape, dis_shape, hidden_size, slot_set, entity_num, embed_size, threshold, self.device, self.max_turn)
        self.target_model = KGRL(GraphAgent, DisInfer, input_shape, sym_shape, dis_shape, hidden_size, slot_set, entity_num, embed_size, threshold, self.device, self.max_turn)
