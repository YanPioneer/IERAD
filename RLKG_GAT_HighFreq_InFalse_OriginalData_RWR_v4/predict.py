...
model_folder = os.path.join(father_path, 'output', 'kgrl_model', '2023.03.21-17-31-38', 'kgrl_model_0.8494.pth.tar')
# model_folder = os.path.join(father_path, 'output', 'kgrl_model', '2023.07.06-22-45-35', 'kgrl_model_0.8368.pth.tar')

timeStr = time.strftime('%Y.%m.%d-%H-%M-%S', time.localtime(time.time()))
writer = SummaryWriter('./runs/{}'.format('KGRL-' + timeStr))
parser = argparse.ArgumentParser()

parser.add_argument('--data_folder', dest='data_folder', type=str, default='./KGData/dataset_gmd/', help='folder for all data')
...
parser.add_argument('--sym_shape', dest='sym_shape', type=int, default=118, help='sym_shape')  # num for symptom
parser.add_argument('--attention_dim', dest='attention_dim', type=int, default=128, help='attention_dim')  # attention dim
parser.add_argument('--dis_shape', dest='dis_shape', type=int, default=12, help='dis_shape')  # num for disease 是否加UNK？？
parser.add_argument('--entity_num', dest='entity_num', type=int, default=130, help='entity_num')  # num for kg entity
...
parser.add_argument('--disease_model', dest='disease_model', type=str, default='./output/useful_models/disease_inference_0.8661.pth.tar', help='trained disease model')  # pretrained model for disease inference
parser.add_argument('--gat_model', dest='gat_model', type=str, default='./output/useful_models/gat_0.7782.pth.tar', help='trained gat model')  # pretrained model for gat
...
goal_set = load_pickle('{}/gmd_new.pk'.format(data_folder))
slot_set = text_to_dict('{}/gmd_slot_set.txt'.format(data_folder))  # all slots with symptoms + all disease
slot_list = text_to_list('{}/gmd_slot_set.txt'.format(data_folder))
sym_dict = text_to_dict('{}/gmd_symptom.txt'.format(data_folder))  # all symptoms
dise_dict = text_to_dict('{}/gmd_disease.txt'.format(data_folder))  # all diseases
